...
from config_loader import CROSSREF_API_MAX_RETRY
from config_loader import WORK_DIR_PATH, JOURNAL_UPDATE_DAYS, JOURNAL_UPDATE_MAX
...
class JournalFetcher:
    ...
    def __init__(self):
        ...
        self.update_days = JOURNAL_UPDATE_DAYS
        self.update_max = JOURNAL_UPDATE_MAX
    ...
    def update_journal_info(self):
        ...
        for i, record in enumerate(to_process):
            ...
            if time.time() - last_display_time > 10:
                ...
                print(f"Status: {remaining} updates remaining out of {total_to_process}...")
                last_display_time = time.time()
        
        # 3. Handle Warning and Flag
        if remaining_updates > 0:
            self.has_pending_updates = True
            logging.warning("Journal update limit reached. %d records still need updating.", remaining_updates)

        self.journal_info_list = final_list

    def load_and_merge_journal_info_list(self, journal_title_list=[], input_filepath=None):
        """Load the list."""
