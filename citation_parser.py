...
class CitationParser:
    ...
    def __init__(self, blacklist=None):
        ...
        last_name = rf"[A-Z][a-zÀ-ÿ]*(?:{dash_class}[A-Z][a-zÀ-ÿ]*)*(?:\s+[A-Z][a-zÀ-ÿ]+)?"
        ...
        self.author_pattern = rf"{name_unit}(?:\s+et\s+al\.|(?:\s+(?:and|et)\s+{name_unit}))?"
    ...
    def extract_citations(self, text):
        ...
        Returns a list of dicts: {'author': str, 'year': str, 'type': 'narrative'|'parenthetical'}
        """
        results = []
        
        # 1. NARRATIVE CITATIONS: Hovius et al. (1997, 1999)
        # Handles one or multiple years inside the parentheses following an author.
        narrative_regex = rf"({self.author_pattern})\s*\((({self.year_pattern})(?:,\s*{self.year_pattern})*)\)"
        ...
        for block in paren_blocks:
            ...
            for group in groups:
                ...
                for word in self.blacklist:
                    clean_group = re.sub(rf"\b{word}\b[. ,]*", "", clean_group, flags=re.IGNORECASE)
